...
#else
    #define THREAD_LOCAL  /* no thread-local support, fallback to nothing */
    #warning "No thread-local storage support detected, not thread safe"
#endif


// Poll global variables 
static THREAD_LOCAL struct pollfd * pollFileDescriptors;
static THREAD_LOCAL int maxFileDescriptor = 0;
static THREAD_LOCAL int currentPollSetSize = 0;
